...
Card play(Input* in) {
  ...
  if (in->currentPlayer == in->currentTrick.startingPlayer) {
    nValidCards = in->nCardsInHand;
    memcpy(validCards, in->cardsInHand, nValidCards * sizeof(Card));
  } else {
    int S = in->nCardsInHand;
    nValidCards = 0;
    for (int i = 0; i < S; ++i) {
      if (in->cardsInHand[i].suit == in->currentTrick.suit) {
        validCards[nValidCards++] = in->cardsInHand[i];
      }
    }
    if (nValidCards == 0) {
      nValidCards = in->nCardsInHand;
  ...
  return validCards[rand() % nValidCards];
}
